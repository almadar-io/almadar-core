...
export function translateOverlaysToParams(
  ...
  applyTheme(presentation, params);
...
/**
 * Lower the plan-level theme name (from `PresentationOverlay.themeRef`)
 * onto `params.theme`. Theme is uniform across orbitals — the coordinator
 * decides it once — so there is no signature validation here (unlike
 * `applyPagePaths`, which checks each page against the signature). The
 * valid-theme enum lives in the factory signature's `paramsSchema`,
 * generated by pattern-sync from the baked `themes-registry.json`.
 */
function applyTheme(
  presentation: PresentationOverlay | undefined,
  params: FactoryCallSiteParams,
): void {
  if (presentation?.themeRef) {
    params.theme = presentation.themeRef;
  }
}
